...
int a_debug = 1;		/* debug level, 0 = quiet, 1 = normal, 2 = debug */
int a_force = 0; 	/* continue despite errors */
volatile char *stackP = NULL;	/* pointer to stack at main, when set activates various memory erasure routines */
...
m_u32 simpleSum(u_char *p, int len)
{
    m_u32 n;
    m_u32 sum = 0;
    for (n=0; n<len; n++)
...
bool waitForEntropy = TRUE;


void maruRandom(void *buf, int n, maru_random type)
{
    int i;
    char *fn;
    char *p = buf;
...
void maruRandomf(void *data, int len, maru_random type)
{
//   va_list ap;
//   va_start(ap, fmt);
//   if (a_debug>0)
...
void maruWipe(void *p, int n)
{
    ...
    for (j=0; j<2; j++)
	    maruRandom(buf, n, RAND_PSEUDO);
}

 u_char *loadFile(char *name, int *len)
{
    struct stat st;
    char *p;
 ...
 maruKeymap *loadKeymap(char *name, int *klenp)
{
    int len;
 ...
 void makeKeymapSum(maruKeymap *h, int len)
{
    h->headSum = hton32(0);
    h->headSum = hton32(simpleSum((u_char*)h, len));
}

 void saveKeymap(char *name, maruKeymap *h, int len)
{
 ...
 void syncInstance(maruInstance *i)
{
 ...
 void freeInstance(maruInstance *i)
{
    int n;
    syncInstance(i);
    if (i->remapDesc->free)
 ...
 */
 ...
 maruAspect *buildAspect(maruInstance *i, maruKeymapAspect *h, int as, maruPass *key, int keylen)
{
    maruCipherDesc *c1;
    int n;
    int iterations;
...
ex:
 ...
 bool validKeymap(maruKeymap *h, int len)
{
    m_u32 sum, sum2;
 ...
 maruInstance *instanceNew(maruKeymap *h, int klen, maruRemapFlags remap_flags)
{
   maruInstance *i;
 ...
 maruAspect *getAspect(maruInstance *i, int as)
{
    if (as >= i->aspects)
...
//Returns the size of a maruKeymapAspect struct added to the size of the 
//bmap remap of the passed maruInstance's remap structure, if applicable.
 int getKeymapAspectLen(maruInstance *i)
{
 ...
 maruKeymapAspect *getKeymapAspect(maruInstance *i, maruKeymap *keymap, int as)
{
    return
	(maruKeymapAspect*)
 ...
 maruInstance *genInstance(char *fn, maruRemapFlags remap_flags)
{
    maruInstance *i;
    maruKeymap *h;
    int n;
 ...
 void listCiphers(FILE *fh)
{
 ...
 maruCipherDesc *xfindCipherTxt(char *txt)
{
 ...
 maruCipherDesc *xfindCipherType(maruCipher cipher)
{
