...
extern void *maruCalloc(int len);
 ...
 void *maruOpaqueInit(maruCipherDesc *cipher)
{
 ...
 void maruOpaqueFree(void *opaque)
{
    maruWipeFree(opaque);
}

 void xor(void *vp, void *vp1, int len)
{
    int n;
    char *p = vp;
    char *p1 = vp1;
...
static m_u64 xor64sumTo(m_u64 *p, m_u64 *p1, m_u64 *to, int len)
{
 ...
 void int2char(m_u32 i, u_char *p)
{
    p[3] = i;
    p[2] = i>>8;
    p[1] = i>>16;
 ...
 m_u32 char2int(u_char *p)
{
    return p[3] | (p[2]<<8) | (p[1] << 16) | (p[0] <<24);
}

 maruCipherDesc *findCipherType(maruCipher cipher)
{
 ...
 maruCipherDesc *findCipherTxt(char *txt)
{
    maruCipherDesc *m = m_ciphers;
    for (; m->cipher != m_none; m++)
 ...
 void maruGenBlockKey(maruAspect *a, maruKey *key, int keylen, m_u32 blockno)
{
    int n;
...
/* --rpw fix: maruEncryptBlock was broken for block != to */
 void maruEncryptBlock(maruAspect *a, u_char *block, u_char *to, int len, m_u32 blockno, int flags)
{
    maruKey key;
