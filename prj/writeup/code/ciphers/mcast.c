...
#include "mcast.h"
 ...
 void CASTinit(maruCipherDesc *cipher, void *opaque, int flags)
{
}

 void CASTsetkey(void *opaque, u_char *key, int len, int flags)
{
...
static void CASTencryptCBC(void *opaque, u_char *iv, u_char *data, int len)
{
 ...
 void CASTencryptCBCTo(void *opaque, u_char *iv, u_char *data, u_char *to, int len)
{
    int n;
    m_u64 *d = (m_u64*)data;
    m_u64 *d2 = (m_u64*)to;
...
static void CASTdecryptCBCTo(void *opaque, u_char *iv, u_char *data, u_char *to, int len)
{
 ...
 void CASTcryptCBC(void *opaque, u_char *iv, u_char *data, u_char *to, int len, int flags)
{
    assert(len % 8 == 0);
    return (data == to)?
	       (flags & MCD_ENCRYPT)?
