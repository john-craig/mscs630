...
/* len must be a multiple of 8 */
 ...
 void IDEAinit(maruCipherDesc *cipher, void *opaque, int flags)
{
}
...
static void IDEAdecryptCBCTo(void *opaque, u_char *iv, u_char *data, u_char *dst, int len)
{
 ...
 void IDEAsetkey(void *opaque, u_char *key, int len, int flags)
{
    return
	(flags & MCD_ENCRYPT)?
	    IDEAsetkeyEnc(opaque, key, len)
 ...
 void IDEAcryptCBC(void *opaque, u_char *iv, u_char *data, u_char *to, int len, int flags)
{
