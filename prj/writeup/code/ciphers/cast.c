/* $Id: cast.c,v 1.1 2000/04/19 16:51:22 proff Exp $
 * $Copyright:$
 */
/*
 *	CAST-128 in C
...
#define U8a(x) ( (m_u8) (x>>24) )
#define U8b(x) ( (m_u8) ((x>>16)&255) )
#define U8c(x) ( (m_u8) ((x>>8)&255) )
#define U8d(x) ( (m_u8) ((x)&255) )
...
m_u32 t, l, r;
	...
	l = ((m_u32)inblock[0] << 24) | ((m_u32)inblock[1] << 16) |
	 ((m_u32)inblock[2] << 8) | (m_u32)inblock[3];
	r = ((m_u32)inblock[4] << 24) | ((m_u32)inblock[5] << 16) |
	 ((m_u32)inblock[6] << 8) | (m_u32)inblock[7];
	/* Do the work */
	F1(l, r,  0);
	F2(r, l,  1);
	F3(l, r,  2);
...
m_u32 t, l, r;
	...
	/* Get inblock into l,r */
	r = ((m_u32)inblock[0] << 24) | ((m_u32)inblock[1] << 16) |
	 ((m_u32)inblock[2] << 8) | (m_u32)inblock[3];
	l = ((m_u32)inblock[4] << 24) | ((m_u32)inblock[5] << 16) |
	 ((m_u32)inblock[6] << 8) | (m_u32)inblock[7];
	/* Do the work */
	/* Only do full 16 rounds if key length > 80 bits */
	if (key->rounds > 12) {
...
int i;
	...
	for (i = 0; i < 4; i++) {
		x[i] = 0;
		if ((i*4+0) < keybytes) x[i] = (m_u32)rawkey[i*4+0] << 24;
		if ((i*4+1) < keybytes) x[i] |= (m_u32)rawkey[i*4+1] << 16;
		if ((i*4+2) < keybytes) x[i] |= (m_u32)rawkey[i*4+2] << 8;
		if ((i*4+3) < keybytes) x[i] |= (m_u32)rawkey[i*4+3];
	}
