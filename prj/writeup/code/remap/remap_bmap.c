...
//I have no idea how in the world this wouldn't result in a segfault
//But my rough understanding is this returns the difference between
//the starting address of a newly-declared 'bmap' struct and the
//address of an element at index 'blocks' in the remap member of that
//'bmap' structure.
//
//So this returns, effectively, however many bytes is between the beginning
//of a 'bmap' structure and the 'block'th element of its 'remap' array-- which
//is a uint32 array. How is this any different than just multiplying 'blocks' by
// sizeof uint32? I have no idea.
m_u32 maruRemapBmapSize(m_u32 blocks)
{
    struct maruRemapBmapKeymap_st bmap;
    return (char*)&bmap.remap[blocks] - (char*)&bmap;
}

maruKeymapAspectRemap* maruRemapBmapCreate(maruInstance *i, m_u32 blocks, int *lenp, maruRemapFlags remap_flags)
{
...
void* maruRemapBmapNew(maruInstance *i, maruRemapFlags remap_flags)
{
...
void* maruRemapBmapAddAspect(maruAspect *a, maruKeymapAspectRemap *hr)
{
...
void maruRemapBmapReleaseAspect(maruAspect *a)
{
    m_u32 n;
    maruInstance *i = a->instance;
...
void maruRemapBmapFree(maruInstance *i)
{
    int n;
    struct remapBmapInstance *bi = i->remapInstanceCtx;
...
bool maruRemapBmapMapIO(maruReq *req)
{
...
void maruRemapBmapInfo(maruInstance *i, int as, maruRemapInfoFlag flag)
{
    switch(flag)
	{
	case I_INSTANCE:
...
bool maruRemapBmapSync(maruInstance *i)
{
    struct remapBmapInstance *bi = i->remapInstanceCtx;
    int n;
