 ...
 */
static void kue_free_kc (struct kue_softc *sc, struct kue_rchain *kc){
    DB("kue_free_kc(%p, %p)\n", sc, kc);
 ...
 */
static int kue_shutdown_sc (struct kue_softc *sc){
...
inline static int kue_readable (struct kue_softc *sc){
...
static int kue_kapi_shutdown (struct kue_kapi *kapi){
 ...
 */
int kue_push (struct kue_kapi *kapi, void *p, int len){
    struct kue_rchain *kc;
 ...
 */
int kue_open (struct inode *i, struct file *f){
    ENTERSC (i->i_rdev);
 ...
 */
int kue_release (struct inode *i, struct file *f){
    ENTERSC (i->i_rdev);
    DB ("kue_release(%p, %p)\n", i, f);
#ifndef KUE_EXCLUSIVE_OPEN
    if (sc->sc_clients > 0)
 ...
 */
ssize_t kue_read (struct file *f, char *buf, size_t n, loff_t * ppos){
    struct kue_rchain *kc;
    int len = 0;
 ...
 */
ssize_t kue_write (struct file *f, const char *buf, size_t n, loff_t * ppos){
    ENTERSC (FINODE(f)->i_rdev);

    DB ("kue_write(%p, %p, %lu, %p)\n", f, buf, n, ppos);
...
long kue_ioctl(struct file *f, unsigned int cmd, unsigned long arg){
...
// #ifndef LINUX20
unsigned int kue_poll (struct file *f, struct poll_table_struct *pt)
{
    unsigned int mask = 0;
...
static const struct file_operations kue_fops =
{
    .read = kue_read,			/* read */
    .write = kue_write,			/* write */
    .poll = kue_poll,			/* poll */
    .unlocked_ioctl = kue_ioctl,	                /* ioctl */
    .open = kue_open,			/* open */
    .release = kue_release,	       	/* release */

};

/* replacement for the NetBSD KUEIOCGKAPI ioctl kludge */
struct kue_kapi *kue_get_kapi (int minor, int reuse)
{
    struct kue_softc *sc;
...
void  kue_release_kapi(struct kue_kapi *kapi) {
    struct kue_softc *sc;
...
int __init kue_drvinit (void) {
    ...
    for (i = 0; i < NR_KUE; i++)
        {
		//fixme
            sema_init(&kue_softc[i].lock_sem,0);
            sema_init(&kue_softc[i].read_sem,1);
        }
...
void __exit kue_drvexit (void) {
    unregister_chrdev (KUE_MAJOR, "kue");
    if (kue_softc)
		kfree (kue_softc);
}

module_init(kue_drvinit);
module_exit(kue_drvexit);
